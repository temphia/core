...
func (d *DynDDL) AddTable(tenantId, gslug string, model *entities.NewTable) error {
...
func (d *DynDDL) AddTableRef(tenantId, gslug string, model *entities.NewTable) (err error) {
	clear := false
	defer func() {
		if clear {
			d.rollbackTableMeta(tenantId, gslug, model.Slug)
