...
type Zenerator interface {
	NewGroup(tenantId string, model *entities.NewTableGroup) (*DDLGroupStmt, error)
	NewTable(tenantId, gslug string, model *entities.NewTable, siblings []string) (*DDLStmt, error)
	DropTable(tenantId, gslug, tslug string) (string, error)

	AddColumn(tenantId, gslug, tslug, cslug string, model *entities.NewColumn) (string, error)
