...
func (d *dynSource) NewGroup(model *entities.NewTableGroup) error {
	ddb := d.dynDB()
...
func (d *dynSource) AddTable(gslug string, model *entities.NewTable) error {
	ddb := d.dynDB()
...
func (d *dynSource) AddColumn(gslug, tslug string, model *entities.NewColumn) error {
	ddb := d.dynDB()
