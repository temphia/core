...
func (s *Seeder) getTable(name string) *entities.NewTable {
...
func (s *Seeder) dataSeed() error {
	...
	bytes, err := s.pacman.blobStore(s.tenant).GetBlob(context.Background(), btypes.BprintBlobFolder, ffile(s.model.ID, "data.yaml"))
	if err != nil {
		return err
	}
