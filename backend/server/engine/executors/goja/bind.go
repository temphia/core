...
func (g *Goja) bind() {
...

	{
		...
		g.qbind("_get_self_file_as_str", func(file string) (interface{}, interface{}) {
			data, err := g.binder.GetSelfFile(file)
			if err != nil {
	...
	if pbind := g.binder.GetPlugKVBindings(); pbind != nil {
	...
	if cbind := g.binder.GetCabinetBindings(); cbind != nil {
		...
		g.qbind("_cab_generate_ticket", func(folder string, opts *rtypes.CabTicket) (interface{}, interface{}) {
...
func http(bindings rtypes.Bindings, runtime *goja.Runtime) func(request *HTTPRequest) *HTTPResponse {
	return func(request *HTTPRequest) *HTTPResponse {
		...
		resp := bindings.HTTPCall(rtypes.HTTPRequest{
